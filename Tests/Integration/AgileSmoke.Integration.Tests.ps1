...
InModuleScope JiraAgilePS {
    Describe "Agile smoke" -Tag 'Integration', 'Smoke', 'Server', 'Cloud' -Skip:$Skip {
            ...
            if (-not $env.ReadOnly) {
                if (-not [string]::IsNullOrEmpty($fixtures.TestProject)) {
                    try {
                        ...
                        Write-Warning "Agile smoke: could not create a temporary issue for write coverage ($($_.Exception.Message))."
                    }
                }
                ...
                $filter = New-JiraFilter `
                    -Name $filterName `
                    -JQL $filterJql `
                    -Description 'Auto-seeded by JiraAgilePS integration tests so Cloud and Data Center run the same board smoke coverage. Safe to delete.' `
                    -Favorite `
                    -ErrorAction Stop
        ...
        It "can list board issues" {
            ...
            { $issues = @(Get-JiraAgileIssue -Board $script:createdBoard -PageSize 1 -ErrorAction Stop) } | Should -Not -Throw
        }
        ...
        It "can list board backlog issues" {
            ...
            { $issues = @(Get-JiraAgileIssue -Board $script:createdBoard -Backlog -PageSize 1 -ErrorAction Stop) } | Should -Not -Throw
        }
